...
Features: TypeAlias = Float[torch.Tensor, "*shape"]
# MaskedFeatures are similar to Features, but are 0 where FeatureMask is False
MaskedFeatures: TypeAlias = Float[torch.Tensor, "*shape"]
FeatureMask: TypeAlias = Bool[torch.Tensor, "*shape"]
SelectionMask: TypeAlias = Bool[torch.Tensor, "*shape"]
# We allow arbitrary labels
Label: TypeAlias = Float[torch.Tensor, "*shape"]
Logits: TypeAlias = Float[torch.Tensor, "*shape"]

# Outputs of AFA methods, representing which feature group to acquire next, or to stop acquiring features (0)
AFAAction: TypeAlias = Integer[torch.Tensor, "*batch 1"]   

# Similar to an AFAAction, but does not include the stop action!
# Unmaskers receive this.
AFASelection: TypeAlias = Integer[torch.Tensor, "*batch 1"] 
...
_DATASET_KEY_ALIASES = {
    ...
    "MimicIVDataset": "mimic_iv",
    "MimicIV5ClassDataset": "mimic_iv_5class",

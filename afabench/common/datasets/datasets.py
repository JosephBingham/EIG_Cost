...
@final
class MimicIVDataset(Dataset[tuple[Tensor, Tensor]], AFADataset):
    """
    MIMIC-IV multi-disease differential diagnosis dataset.

    21 acute conditions, 55 features grouped into 30 test groups
    with real 2026 Medicare CPT costs.
    """
    ...
    @override
    def feature_shape(self) -> torch.Size:
        return torch.Size([getattr(self, "n_features", 55)])

    @property
    @override
    def label_shape(self) -> torch.Size:
        return torch.Size([getattr(self, "n_classes", 21)])
    ...
    def __init__(self, path: str = "extra/data/mimic_iv"):
        super().__init__()
        self.path = path
        ...
        if not (data_dir / "X_train.npy").exists():
            msg = (
                f"MIMIC-IV dataset not found at {self.path}. "
                "Run the export notebook first to generate the data files."
            )
            raise FileNotFoundError(msg)
        ...
        # Load all splits and concatenate (the benchmark handles splitting)
        X_parts = []
        y_parts = []
        ...
        for split in ["train", "val", "test"]:
            ...
            if x_path.exists() and y_path.exists():
                X_parts.append(np.load(str(x_path)))
                y_parts.append(np.load(str(y_path)))
        ...
        # Z-normalize
        '''
        eatures_df = pd.DataFrame(X_all)
        features_df = features_df.fillna(features_df.mean())
        features_df = _z_normalize(features_df)

        self.features = torch.tensor(features_df.values, dtype=torch.float32)
        '''
        # Data is already standardized from the export notebook
        # Just handle any remaining NaNs
        ...
        # Per-feature costs from groups.json
        self._feature_costs = torch.tensor(
            self.groups_info["group_costs"],
            dtype=torch.float32,
        )
        self._feature_to_group = self.groups_info["feature_to_group"]

    @override
    def get_feature_acquisition_costs(self) -> torch.Tensor:
        """Return per-feature costs based on CPT group costs."""
        costs = torch.zeros(self.n_features, dtype=torch.float32)
        group_costs = self.groups_info["group_costs"]
        for feat_idx, group_idx in enumerate(self._feature_to_group):
            costs[feat_idx] = group_costs[group_idx]
        return costs
    ...
    def load(cls, path: Path) -> Self:
        ...
        obj.class_names = data.get("class_names", [])
        obj.groups_info = data.get("groups_info", {})
        obj.metadata = data.get("metadata", {})
        obj.n_features = obj.features.shape[1]
        obj.n_classes = obj.labels.shape[1]
        obj._feature_costs = torch.tensor(
            obj.groups_info.get("group_costs", [1.0] * obj.n_features),
            dtype=torch.float32,
        )
        obj._feature_to_group = obj.groups_info.get(
            "feature_to_group", list(range(obj.n_features))
        )
        return obj

        
@final
class MimicIV5ClassDataset(MimicIVDataset):
    """5-class subset of MIMIC-IV (DKA, MI, pancreatitis, sepsis, stroke).

    Identical layout to MimicIVDataset, just a different default path.
    ESR feature dropped (100% missing in MIMIC-IV). Imbalance preserved.

    Build the underlying data files with build_cohort.py.
    """

    def __init__(self, path: str = "extra/data/mimic_iv_5class"):
